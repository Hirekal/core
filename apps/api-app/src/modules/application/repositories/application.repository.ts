...
export class ApplicationRepository extends BaseRepository<Application> {
   ...
   */
  async findByIdWithWebhookRelations(id: string): Promise<Application | null> {
    return this.repository.findOne({
   ...
   */
  async listForJob(options: ApplicationListOptions): Promise<Application[]> {
    ...
    switch (options.sortBy) {
      case ApplicationSortBy.NAME:
        qb.orderBy('application.firstName', 'ASC').addOrderBy(
      ...
      default:
        qb.orderBy('application.submittedAt', 'DESC', 'NULLS LAST').addOrderBy(
