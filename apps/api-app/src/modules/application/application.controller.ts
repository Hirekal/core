...
export class ApplicationController {
  ...
  async findOne(
    ...
    try {
      return await this.applicationService.findByIdForOrg(id, organizationId);
    } catch (error) {
      this.logger.error(
  ...
  async updateStage(
    ...
    try {
      ...
      this.logger.error(`Update stage ${id} failed: ${toErrorMessage(error)}`);
      throw error;
    }
  }
  ...
  async updateRating(
    ...
    try {
      ...
      this.logger.error(`Update rating ${id} failed: ${toErrorMessage(error)}`);
      throw error;
    }
  }
