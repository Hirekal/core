...
export class PublicApplicationJobController {
  ...
  async start(@Param('slug') slug: string, @Body() dto: StartApplicationDto) {
    try {
  ...
  @Public()
  @Post(':slug/view')
  async trackView(@Param('slug') slug: string, @Body() dto: TrackJobViewDto) {
    try {
      return await this.applicationService.trackPageView(slug, dto.sessionId);
    } catch (error) {
      this.logger.error(
