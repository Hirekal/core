...
export class JobService {
                ...
                try {
                    const jobId = await this.dataSource.transaction(async (manager) => {
                        ...
                        const job = jobRepo.create({
                            ...
                            questionRetakes:
                                dto.questionRetakes ?? QuestionRetakes.UNLIMITED,
                            transcriptionLanguage:
                                dto.transcriptionLanguage ?? 'english',
    ...
    async update(
        ...
        try {
            ...
            if (error instanceof HttpException) throw error;
            this.logger.error(
                `update failed id=${id}: ${(error as Error).message}`,
            );
    ...
    async delete(
        ...
        try {
            ...
            if (error instanceof HttpException) throw error;
            this.logger.error(
                `delete failed id=${id}: ${(error as Error).message}`,
            );
    ...
    async duplicate(
        ...
        try {
            ...
            let introMediaType = original.introMediaType;
            let introMediaStorageKey: string | null = null;
            let introMediaUrl: string | null = null;
            let introMediaFileName = original.introMediaFileName;
                    ...
                    await this.dataSource.transaction(async (manager) => {
                        ...
                        await settingsRepo.save({
                            ...
                            emailAutomation: structuredClone(
                                original.settings?.emailAutomation ??
                                DEFAULT_EMAIL_AUTOMATION,
                            ),
    ...
    async pause(
        ...
        return this.transitionStatus(id, organizationId, userId, JobStatus.ACTIVE, JobStatus.PAUSED);
    }
    ...
    async resume(
        ...
        return this.transitionStatus(id, organizationId, userId, JobStatus.PAUSED, JobStatus.ACTIVE);
    }
    ...
    async preview(
        ...
        try {
            ...
            if (error instanceof HttpException) throw error;
            this.logger.error(
                `preview failed id=${id}: ${(error as Error).message}`,
            );
    ...
    async deleteIntroMedia(
        ...
        try {
            ...
            throw new InternalServerErrorException(JobErrors.FAILED_TO_DELETE_INTRO_MEDIA);
        }
    }
